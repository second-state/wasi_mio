...
impl SelectorState {
    pub fn new() -> io::Result<SelectorState> {
        Ok(Self {
            ...
            notify_waker: AtomicUsize::new(1),
            ...
            if num_fd_events > 0 {
                ...
                for fd_data in fds.fd_data.values_mut() {
                    ...
                    if poll_fd.revents != 0 {
                        ...
                        if poll_fd.events == 0 {
                            poll_fd.events = libc::POLLOUT | libc::POLLIN;
                        }
    ...
    fn modify_fds<T>(&self, f: impl FnOnce(&mut Fds) -> T) -> T {
        ...
        self.notify_waker.fetch_add(1, Ordering::SeqCst);
